...
function WeatherControlComponent({ nodePath, client, endpoints }: WeatherControlComponentProps) {
  ...
  const applyPreset = async (preset: WeatherPreset) => {
    ...
    try {
      ...
      // Find matching endpoints for preset values
...
interface WeatherControlComponentProps {
  nodePath: string;
  client: TSWClient | null;
...
function NodeValuesPanel({ nodePath, client }: NodeValuesPanelProps) {
  ...
  const shouldShowWeatherControls = useCallback((nodePath: string | null, nodeData: TWSApiResponse | null): boolean => {
    ...
    // Check if the node path contains WeatherManager
    const isWeatherManager = nodePath.toLowerCase().includes('weathermanager');

    // Check if there are writable endpoints
  ...
  // Check for weather controls when nodeData changes
  useEffect(() => {
